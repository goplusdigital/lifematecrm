...
export async function GET(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const couponId = parseCouponId(rawId);
    if (!couponId) {
    ...
    const couponRes = await pool.query(
      ...
      WITH claim_counts AS (
        ...
        FROM coupon_claims
        WHERE member_code = $1 AND coupon_id = $2
    ...
    const row = couponRes.rows[0] as Record<string, unknown>;
    ...
    } else if (get("is_active") === false) {
      canClaim = false;
      disabledReason = "คูปองนี้ถูกปิดใช้งานแล้ว";
    }
    ...
    const data = {
      ...
      usageConditions: String(get("usage_conditions") ?? ""),
