...
const EmptyState = ({ message }: { message: string }) => (
  <div className="flex flex-col items-center justify-center bg-[#f5f5f5] m-0 p-8 relative h-dvh w-full">
    ...
    <p className="text-sm font-prompt text-gray-700">{message}</p>
  </div>
)
...
export default function Shopping() {
  ...
  const formatSold = (quantity: number) => {
    ...
    return `${new Intl.NumberFormat('th-TH').format(value)} ${t('sold')}`
  }
    ...
    return (
      <div className="min-h-screen bg-[#f5f5f5]">
        ...
        <div className="flex flex-col items-center justify-center h-[70vh]">
          <p className="text-sm font-prompt text-gray-400">{t('loading')}</p>
        </div>
      </div>
    )
  }
  ...
  if (products.length === 0) {
    return (
      <div className="min-h-screen bg-[#f5f5f5]">
        {topSearchBar}
        <EmptyState message={t('no_products')} />
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-[#f5f5f5]">
      ...
      <div className="bg-white px-4 py-4 border-b border-gray-200">
        <div className="max-w-md mx-auto">
          <div className="text-gray-800 font-prompt text-sm font-semibold">{t('deals_title')}</div>
          <div className="text-gray-500 font-prompt text-xs mt-1">{t('product_count', { count: products.length })}</div>
          <div className="mt-3 flex gap-2 overflow-x-auto pb-1 scrollbar-hide [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            <button type="button" onClick={() => setActiveCategoryId('all')} className={chipClassName(activeCategoryId === 'all')}>
              {t('all_categories')} ({new Intl.NumberFormat('th-TH').format(totalCategoryCount)})
            </button>
