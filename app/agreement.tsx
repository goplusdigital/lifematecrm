    ...
    <div className="flex flex-col h-dvh w-full overflow-hidden bg-gray-50">
      ...
      <header className="flex-none p-2 bg-white border-t border-gray-100">
        <div className="flex justify-between gap-2">
          ...
          <div className="flex justify-end gap-2 mb-2">
            <button onClick={() => setLocale('th')} className={`px-3 py-1 text-sm font-prompt  rounded ${locale == 'th' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}>
              ไทย
            </button>
            <button onClick={() => setLocale('en')} className={`px-3 py-1 text-sm font-prompt  rounded ${locale == 'en' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}>
              ENG
            </button>
          </div>
        </div>
      ...
      <footer className="flex-none p-4 bg-white border-t border-gray-100 space-y-4">
        <div className="flex items-start gap-3">
          <input type="checkbox" id="agree_pdpa" className="mt-1 h-5 w-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500" onChange={(e) => setAcceptedPDPA(e.target.checked)} />
        ...
        <div className="flex items-start gap-3">
          <input type="checkbox" id="agree_marketing" className="mt-1 h-5 w-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500" onChange={(e) => setAcceptMarketting(e.target.checked)} />
        ...
        <button
          ...
          onClick={() => { doContinue() }}
          className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-lg transition-colors font-prompt disabled:opacity-50">
          {t('continue')}
        </button>
      </footer>
